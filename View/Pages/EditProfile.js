...
const API_ENDPOINT = new URL(
  "../../Controller/EditProfileController.php",
...
async function loadUsers() {
  try {
    ...
    renderUsers();
    showToast(
      "⚠️ Could not load users from the database. Using mock data.",
      true,
    );
  }
}
...
function clearAddForm() {
  ["newFirst", "newLast", "newEmail", "newPassword", "newClass"].forEach(
    (id) => {
...
async function addUser() {
  ...
  if (!password || password.length < 8) {
    showToast("⚠️ Please enter a password of at least 8 characters.", true);
    return;
  }
...
function setModalMode(mode, id = null) {
  ...
  if (mode === "update") {
    ...
    document.querySelector(".ep-modal__header h3").textContent =
      "➕ Add New User";
  }
}

async function confirmAddHandler() {
  const addBtn = document.getElementById("confirmAdd");
  if (!addBtn) {
    return;
  }
  ...
  const id = Number(addBtn.dataset.userId || 0);
...
async function updateUser(id) {
  ...
  const first = document.getElementById("newFirst").value.trim() || u.first;
  const last = document.getElementById("newLast").value.trim() || u.last;
  const email = document.getElementById("newEmail").value.trim();
  const password = document.getElementById("newPassword").value;
  const role = document.getElementById("newRole").value;
  const cls = document.getElementById("newClass").value.trim();
  ...
  if (password && password.length > 0 && password.length < 8) {
    showToast("⚠️ Password must be at least 8 characters.", true);
    return;
  }
  ...
  try {
    ...
    const response = await apiRequest("PUT", body);
    ...
    resetAddModal();
    renderUsers();
    showToast(`✅ ${updated.first} ${updated.last} updated!`);
  } catch (error) {
    showToast(error.message, true);
  }
}
...
async function executeDelete() {
  ...
  if (deleteTarget.type === "user") {
    ...
    try {
      await apiRequest("DELETE", null, `?id=${deleteTarget.id}`);
...
window.resetPersonal = function () {
  ...
  document.getElementById("bio").value =
    "Passionate early childhood educator with 12 years of experience nurturing young learners.";
...
window.changePassword = function () {
  ...
  if (!curr) {
    showToast("⚠️ Enter your current password.", true);
    return;
  }
  if (newPwd.length < 8) {
    showToast("⚠️ New password must be at least 8 characters.", true);
    return;
  }
  if (newPwd !== confirm) {
...
function initPwdToggles() {
  ...
  if (newPwdInput) {
    newPwdInput.addEventListener("input", () =>
      updatePwdStrength(newPwdInput.value),
    );
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    ...
    ?.addEventListener("click", confirmAddHandler);
