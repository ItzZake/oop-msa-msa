...
function showToast(msg, isError) {
  ...
  toast.innerHTML =
    ...
    (isError ? "❌" : "✅") +
    '</span><span style="font-size:0.875rem;font-weight:600;color:var(--gray-800);">' +
    msg +
    "</span>";
...
function switchLoginTab(tab) {
  ...
  if (tab === "login") {
    ...
    loginBtn.style.boxShadow = "0 1px 4px rgba(0,0,0,0.1)";
    regBtn.style.background = "transparent";
    regBtn.style.color = "var(--gray-500)";
    ...
    regBtn.style.color = "var(--blue)";
    regBtn.style.boxShadow = "0 1px 4px rgba(0,0,0,0.1)";
    loginBtn.style.background = "transparent";
    loginBtn.style.color = "var(--gray-500)";
...
function toggleBilling() {
  ...
  document.getElementById("monthlyLabel").style.color = billingYearly
    ? "var(--gray-400)"
    : "var(--pink)";
  document.getElementById("yearlyLabel").style.color = billingYearly
    ? "var(--pink)"
    : "var(--gray-400)";
  document.getElementById("saveBadge").style.display = billingYearly
    ? ""
    : "none";
  const idx = billingYearly ? 1 : 0;
  document.getElementById("basic-price").textContent =
    "$" + prices.basic[idx].toLocaleString();
  document.getElementById("premium-price").textContent =
    "$" + prices.premium[idx].toLocaleString();
  document.getElementById("elite-price").textContent =
    "$" + prices.elite[idx].toLocaleString();
}
...
function setDashNav(btn, section) {
  ...
  if (topbar)
    topbar.textContent = section.charAt(0).toUpperCase() + section.slice(1);
}
...
function setAttView(view) {
  ["teacher", "parent", "admin"].forEach((v) => {
    document.getElementById("att-view-" + v).style.display =
      v === view ? "" : "none";
  ...
  if (view === "teacher") {
    document.getElementById("att-tab-teacher").style.background = "var(--blue)";
    document.getElementById("att-tab-teacher").style.color = "white";
  }
}
...
function setAssignView(view) {
  ["teacher", "parent", "child"].forEach((v) => {
    document.getElementById("assign-view-" + v).style.display =
      v === view ? "" : "none";
    const tab = document.getElementById("assign-tab-" + v);
    if (!tab) return;
    if (v === "teacher") {
      tab.style.background = v === view ? "var(--blue)" : "#EFF6FF";
      tab.style.color = v === view ? "white" : "var(--blue)";
    } else if (v === "parent") {
      tab.style.background = v === view ? "var(--green)" : "#F0FDF4";
      tab.style.color = v === view ? "white" : "var(--green)";
    } else {
      tab.style.background = v === view ? "var(--pink)" : "#FFF0F7";
      tab.style.color = v === view ? "white" : "var(--pink)";
    }
  });
}
...
function showProfileRole(role) {
  ["teacher", "admin", "parent", "child"].forEach((r) => {
    ...
    if (!tab) return;
    const colors = {
      teacher: ["var(--blue)", "#EFF6FF"],
      admin: ["var(--pink)", "#FFF0F7"],
      parent: ["var(--green)", "#F0FDF4"],
      child: ["var(--yellow)", "#FFFBEB"],
    };
...
function setExcuseTab(tab) {
  ["submit", "history"].forEach((t) => {
    document.getElementById("excuse-view-" + t).style.display =
      t === tab ? "" : "none";
...
function filterExcuses(q) {
  ...
  items.forEach((item) => {
    ...
    item.style.display =
      !q || name.includes(q) || reason.includes(q) ? "" : "none";
  });
}
...
function selectChat(el, name, avatar, role, online) {
  ...
  document.getElementById("chatHeaderAvatar").innerHTML =
    avatar +
    (online
      ? '<span style="position:absolute;bottom:0;right:0;width:0.625rem;height:0.625rem;background:var(--green);border-radius:50%;border:2px solid white;"></span>'
      : "");
}
function filterChats(q) {
  document.querySelectorAll(".chat-item").forEach((item) => {
    const name =
      item.querySelector('[style*="font-weight:700"]')?.textContent || "";
    item.style.display =
      !q || name.toLowerCase().includes(q.toLowerCase()) ? "" : "none";
  });
}
