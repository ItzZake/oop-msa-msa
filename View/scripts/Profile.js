...
function renderTeacherContent(tab) {
	try {
		...
		const teacherStats = profileData?.teacherStats || {};
		...
		const overviewCards = [
			{
				...
				value: teacherStats.totalStudents ?? studentsList.length,
				icon: "👦",
				color: "#1565C0",
				bg: "#EFF6FF",
			},
			{
				label: "Present Today",
				value: teacherStats.presentToday ?? 0,
				icon: "✅",
				color: "#10B981",
				bg: "#F0FDF4",
			},
			{
				label: "Absent Today",
				value: teacherStats.absentToday ?? 0,
				icon: "❌",
				color: "#EF4444",
				bg: "#FEF2F2",
			},
			{
				label: "Late Today",
				value: teacherStats.lateToday ?? 0,
				icon: "⏰",
				color: "#F59E0B",
				bg: "#FFFBEB",
			},
...
function renderTeacher() {
	try {
		...
		console.log("renderTeacher - teacherStats:", teacherStats); // DEBUG
		...
		const totalStudents = teacherStats.totalStudents ?? studentsList.length;
		const attendanceRate =
			teacherStats.attendanceRate !== undefined
    ...
    <div class="left-panel">
      <div class="profile-card">
        ...
        <div class="profile-body">
          ...
          <div class="info-list">
            ...
            <div class="info-item" style="color:#1565C0"><span class="info-icon">👥</span> ${totalStudents} Students</div>
      ...
      <div class="stats-card">
        <h4>Quick Stats</h4>
        ${[
					{ label: "Attendance Rate", value: attendanceRate, color: "#10B981" },
...
function renderParent() {
	const parentData = profileData?.parentData || profileData?.userData || {};
	const childrenList = profileData?.childrenList || [];
	const displayName = `${parentData.firstname || "Parent"}${parentData.Lastname ? " " + parentData.Lastname : ""}`;
	const email = parentData.email || "";
	const phone = parentData.phone || "";
	const address = parentData.address || "Not specified";
	const hasChildren = childrenList.length > 0;
	const childCards = hasChildren
		? childrenList
				.map(
					(child) => `
            <div class="child-item">
              <div>
                ...
                <div class="child-meta">${child.gender || "N/A"} · ${child.dateOfBirth || "DOB missing"}</div>
              </div>
              <span class="badge-pill" style="background:#10B981;color:white;width:auto">${child.enrollmentStatus || "Pending"}</span>
            </div>`,
				)
				.join("")
		: `<div class="empty-state">No linked children found. Add a child from the Profiles page.</div>`;

	return `<div class="profile-layout">
    <!-- LEFT -->
    <div class="left-panel">
      <div class="profile-card">
        <div class="profile-banner" style="background:linear-gradient(135deg,#10B981,#059669)">
          <span class="profile-banner-emoji">❤️</span>
        </div>
        <div class="profile-body">
          <div class="avatar">
          ...
          <div style="font-size:0.75rem;color:#9ca3af">Profile details from your account</div>
        </div>
        <div style="display:flex;flex-direction:column;gap:0.5rem;padding:1rem">
          <div class="info-item" style="color:#1565C0;font-size:0.875rem"><span class="info-icon">✉️</span> ${email || "Not provided"}</div>
          <div class="info-item" style="color:#1565C0;font-size:0.875rem"><span class="info-icon">📞</span> ${phone || "Not provided"}</div>
          <div class="info-item" style="color:#1565C0;font-size:0.875rem"><span class="info-icon">🏠</span> ${address}</div>
        </div>
      </div>
      ...
      <div class="stats-card">
        ...
        ${[
					{ label: "Children", value: childrenList.length, color: "#10B981" },
					{
						label: "Linked Profiles",
						value: hasChildren ? "Active" : "Pending",
						color: "#1565C0",
					},
				]
					.map(
    ...
    <div class="right-panel" style="display:flex;flex-direction:column;gap:1.25rem">
      <div class="calendar-card">
        <h4 class="calendar-title">📅 Recent Parent Activity</h4>
        <p style="margin: 0; color: #4b5563;">This section shows your most recent profile activity and reminders.</p>
        <div class="activity-list" style="margin-top:1rem">
          <div class="activity-item">
            <span class="activity-emoji">📢</span>
            <div>
              <div class="activity-name">Check your child’s new class schedule</div>
          ...
          <div class="activity-item">
            <span class="activity-emoji">📝</span>
            <div>
              <div class="activity-name">Update emergency contact details</div>
...
async function loadProfileData() {
	try {
		const response = await fetch(
			"../Controller/ProfileController.php?action=get",
			{
				method: "get",
				credentials: "include",
			},
		...
		if (data.success && data.data) {
			if (data.data.userRole) {
				data.data.userRole = data.data.userRole.toLowerCase();
			}
			console.log("Loaded profile data:", data.data); // DEBUG
			return data.data;
		} else {
...
document.addEventListener("DOMContentLoaded", async () => {
	try {
		...
		const profileRole =
			window.CURRENT_PROFILE_ROLE || profileData.userRole || "teacher";
		...
		errorDiv.style.cssText =
			"position: fixed; top: 10px; right: 10px; background: #ffcccc; padding: 15px; border: 2px solid red; font-size: 12px; max-width: 350px; z-index: 9999; font-family: monospace;";
